...
let
  ...
  inherit (lib) types mkEnableOption mkOption mkIf mapAttrsToList concatStringsSep all attrNames concatMapStringsSep;
  ...
  attrsOfKeys = keyType: elemType:
    ...
    in t // {
      ...
      description = "attribute set with ${keyType.description} as key and ${elemType.description} as value";
    };
  concatMapAttrsToStringSep = sep: f: v: concatStringsSep sep (mapAttrsToList f v);
...
in
{
  options.programs.firefox.tridactyl = {
    ...
    autoCommands = mkOption {
      type = attrsOf (attrsOfKeys
        # Copied from :help
        (enum [ "TriStart" "DocStart" "DocLoad" "DocEnd" "TabEnter" "TabLeft" "FullscreenChange" "FullscreenEnter" "FullscreenLeft" "AuthRequired" "BeforeRedirect" "BeforeRequest" "BeforeSendHeaders" "Completed" "ErrorOccured" "HeadersReceived" "ResponseStarted" "SendHeaders" ])
        (listOf str)
      );
      default = {};
    };
  ...
  config = mkIf cfg.enable {
    ...
    xdg.configFile."tridactyl/tridactylrc".text = ''
      ...
      ${concatMapAttrsToLines (k: concatMapAttrsToLines (k2: (concatMapStringsSep "\n" (v: "autocmd ${k2} ${k} ${v}")))) cfg.autoCommands}

      ${concatMapAttrsToLines (k: v: "bind ${k} ${v}") cfg.bindings}
      ${concatMapAttrsToLines (k: concatMapAttrsToLines (k2: v: "bindurl ${k} ${k2} ${v}")) cfg.urlBindings}
